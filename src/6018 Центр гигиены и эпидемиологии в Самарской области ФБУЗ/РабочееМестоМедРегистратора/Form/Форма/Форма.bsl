...
Процедура ТЗ_НаправленияФамилияПриИзменении(Элемент)
	...
	Если ТД <> Неопределено Тогда
	   ...
	   ТД.ФИО = ТД.Фамилия + " " + ТД.Отчество + " " + ТД.Имя;
...
Процедура РаспечататьДокументы(Команда)
	...
	Если ЗначениеЗаполнено(ИменаКоманд.ДоговорФизЛицо) Тогда
...
Функция ИменаКомандПечати()
	...
	ИменаКоманд = Новый Структура("ДоговорФизЛицо, Приложение");
	...
	Если ЗначениеЗаполнено(ОбработкаПечатиСпракиСчет) Тогда
		ИменаКоманд.Приложение		= "ЩелчокПоКнопкеДопОтчетов_" + СтрЗаменить(строка(ОбработкаПечатиСпракиСчет.УникальныйИдентификатор()),"-","_");
	КонецЕсли;
