...
Функция ФормированиеДоговора(СсылкаДокумент)
   ...
   //Если Объект.Контракт  Тогда        	
	...
	//Пункт31 = "";
	Область31.Параметры.ТекстПункта31 = "3.1. Цена контракта составляет " + СуммаЧислоСтрокой +" (" + СуммаПрописью +"), в т.ч. НДС " + НДСЧислоСтрокой + " (" +НДСПрописью +"). Цена по данному контракту определяется в соответствии с Приложением № " + НомерПунктовПрейскуранта +"  действующего Прейскуранта цен Исполнителя от 01.01.2016г. " ; 
	...
	Область5.Параметры.Пункт5 = "5. Ответственность сторон:";      
	ТекстПункта51 = "5.1. Ответственность Сторон за неисполнение или ненадлежащее исполнение обязательств по контракту определяется в соответствии с действующим законодательством Российской Федерации.";
