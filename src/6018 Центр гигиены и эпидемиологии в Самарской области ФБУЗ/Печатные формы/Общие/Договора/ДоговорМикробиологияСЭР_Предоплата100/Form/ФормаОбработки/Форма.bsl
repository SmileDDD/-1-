...
Процедура Команда1(Команда)
	...
	Если НЕ Объект.ОтменаПечати  Тогда
		...
		Если ТабДок <> Неопределено  Тогда 
			...
			//ТабДок.Показать();
			_ИМЦ_МодульФайлыПечатьКлиент.СозданиеФайлаПриПечатиИзВнешнихОбработок(ТабДок,Объект.ССылкаНаОбъект, ПрефиксНаименованияФайла, ЭтаФорма, Неопределено);
...
Функция ФормированиеДоговора(СсылкаДокумент)
	...
	Область31.Параметры.ТекстПункта31 = "3.1. Цена по данному договору определяется в соответствии с Приложением "
		 + НомерПунктовПрейскуранта + "1 и рассчитывается на основании Прейскуранта цен с 01.01.2016г. на платные медицинские работы (услуги), выполняемые Исполнителем на основании заявки Заказчика на виды лабораторных исследований и составляет до " 
		 + СуммаЧислоСтрокой +" (" + СуммаПрописью +"), в т.ч. сумма НДС.";
	
	//Область31.Параметры.ТекстПункта31 = "3.1. Цена договора составляет " 
	//	+ СуммаЧислоСтрокой +" (" + СуммаПрописью +"), в т.ч. НДС " 
	//	+ НДСЧислоСтрокой + " (" +НДСПрописью +"). Цена по данному договору определяется в соответствии с Приложением № " + НомерПунктовПрейскуранта +"  действующего Прейскуранта цен Исполнителя от 01.01.2016г. " ; 
	...
	ДатаОкончания = Формат(СсылкаДокумент.ДатаОкончания, "ДФ=; ДЛФ=DD");
	Область71.Параметры.ТекстПункта71 = "7.1. Договор вступает в силу с момента его подписания Сторонами и действует до "
		+ ДатаОкончания + ". Окончание срока действия договора не влечет прекращения обязательств по договору и не освобождает Стороны от ответственности за его нарушение.";
