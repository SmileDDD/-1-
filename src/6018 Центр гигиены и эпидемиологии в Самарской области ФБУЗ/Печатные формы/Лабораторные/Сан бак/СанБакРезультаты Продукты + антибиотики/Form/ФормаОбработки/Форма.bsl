...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(ССылкаНаОбъект) Тогда
	   ...
	   //    
	   //    //ПрефиксНаименованияФайла = СтрЗаменить(Строка(Объект.СсылкаНаОбъект),":","."); 				
	   //    ИмяВремФайлаБезПути             = СтрЗаменить(ИмяВрем,КаталогВременныхФайлов(),"");             	
	   //    ИмяФайлаДляПроверкиНаличияВБазе = ПрефиксНаименованияФайла + "_"+ Формат(ТекущаяДата(), "ДФ=ггММддЧЧммсс") + " " + ИмяВремФайлаБезПути;
	   //    
...
Функция ПечатьТабДок(СсылкаНаОбъект)
	...
	Для каждого Стр из Найденные Цикл
		Если Найти(Строка(Стр.ЖурналИсследований),"антибиотик") = 0 Тогда
			Если Первая Тогда //выводим шапку таблицы
				...
				ОбластьТаблицаШапка.Параметры.ДатаРезультата = Строка(Формат(Стр.ДатаРезультата, "ДЛФ = ДД"));
	...
	Для каждого Стр из Найденные Цикл
		Если Найти(Строка(Стр.ЖурналИсследований),"антибиотик") <> 0 Тогда
			Если Первая Тогда //выводим шапку таблицы
				...
				//ОбластьТаблицаШапка2.Параметры.ДатаПосева2 	   = Строка(Формат(Стр.ДатаИсследования, "ДЛФ = ДД"));
                ОбластьТаблицаШапка2.Параметры.ДатаРезультата2 = Строка(Формат(Стр.ДатаРезультата,   "ДЛФ = ДД")); 
	...
	ОбластьСписокПодписывающих.Параметры.ДатаВыдачи = Формат(ТекущаяДата(), "ДЛФ = ДД"); 
	...
	КонецЕсли;  	
	ТабДок.НижнийКолонтитул.ТекстСправа =  "Страница _____ из _____";
