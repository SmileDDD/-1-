...
функция ПечатьТабДок()
	...
	Если ЗначениеЗаполнено(Час(Объект.СсылкаНаОбъект.ДатаИВремяИсследования)) Тогда
		ОбластьШапка.Параметры.ДатаИзмерений	= Формат(Объект.СсылкаНаОбъект.ДатаИВремяИсследования,"ДФ='dd.MM.yyyy ""г.,"" ЧЧ ""ч"" мм ""мин""'");
	Иначе
	...
	ПрефиксНаименованияФайла = "ЭМИ_ПВЭМ_" + СтрЗаменить(Объект.СсылкаНаОбъект.НомерВЖурнале,"/", "") + Формат(Объект.СсылкаНаОбъект.Дата,"ДФ='""_от_""dd_MM_yy'");
	...
	ВремТЗ_МестаИзм       = Объект.СсылкаНаОбъект.Показатели.Выгрузить(,
	...
	Для каждого СтрМестоИзм Из ВремТЗ_МестаИзм  Цикл
		...
		НайденныйВыс = Объект.СсылкаНаОбъект.Показатели.НайтиСтроки(Новый Структура("ГУИД", СтрМестоИзм.ГУИД));
