...
Функция ФормированиеТабличногоДокумента_Протокола(СсылкаНаОбъект, ТабДок)  Экспорт
	...
	Если ЗначениеЗаполнено(Час(СсылкаНаОбъект.ДатаИВремяИсследования)) Тогда
		ОбластьШапка.Параметры.ДатаИзмерений	= Формат(СсылкаНаОбъект.ДатаИВремяИсследования,"ДФ='dd.MM.yyyy ЧЧ ""г.,"" ""ч"" мм ""мин""'");
	Иначе
	...
	ПрефиксНаименованияФайла = "Шум_" + СтрЗаменить(СсылкаНаОбъект.НомерВЖурнале,"/", "") + Формат(СсылкаНаОбъект.Дата,"ДФ='""_от_""dd_MM_yy'");
	...
	НТД ="";
	Для каждого СтрМетодика Из СсылкаНаОбъект.МетодикиОтбора  Цикл
		
		Если ЗначениеЗаполнено(НТД) Тогда
			
			НТД = НТД + ";
			|" +  Строка(СтрМетодика.Методика.ПолноеНаименование);
			
		Иначе	
			НТД = СтрМетодика.Методика.ПолноеНаименование;			
		КонецЕсли;
		
	КонецЦикла;
	
	ОбластьНД.Параметры.НД = НТД;
	...
	ОбластьПДУ.Параметры.Текст = "ПДУ напряженности ЭМП промышленной частоты 50 Гц";
	ДобавлениеОбластиВструктуру(сч, ВозрСтрукт,ОбластьПДУ);
	
	
	
	ВремТЗ_ТочкиИзм       = СсылкаНаОбъект.Показатели.Выгрузить(,
	"ГУИД, Комментарий, СоответствуетНормам, ТипТочкиИзмерения, МестоИзмерения, ТочкаИзмерения");
	ВремТЗ_ТочкиИзм.Свернуть(
	"ГУИД, Комментарий, СоответствуетНормам, ТипТочкиИзмерения, МестоИзмерения, ТочкаИзмерения");
	
	ВремТЗ_МестоИзм       = СсылкаНаОбъект.Показатели.Выгрузить(," МестоИзмерения");
	...
	Для Каждого СтрМестрИзм  Из ВремТЗ_МестоИзм Цикл
		...
		Если НайденныеТочкиИзм.Количество() Тогда
			...
			Для каждого СтрТочкаИзм Из НайденныеТочкиИзм  Цикл
				...
				НайденныйВыс = СсылкаНаОбъект.Показатели.НайтиСтроки(Новый Структура("ГУИД", СтрТочкаИзм.ГУИД));
				...
				Если НайденныйВыс.Количество() Тогда
					...
					ДобавлениеОбластиВструктуру(сч, ВозрСтрукт,ВремТабДок);
	...
	ВозрСтрукт["ТабДок" + сч].Вывести(ОбластьПункт10);
		...
		Если НайденныеТочкиИзм.Количество() Тогда
			...
			Для каждого СтрТочкаИзм Из НайденныеТочкиИзм  Цикл
				...
				Если НайденныйВыс.Количество() Тогда
					...
					ОбъекдОбластьТочкаИзмерения = ВремТабДок.Область(1  , 3,  КолвоВысот  ,27);
					ОбъекдОбластьТочкаИзмерения.Объединить();      					
